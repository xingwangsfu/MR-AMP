...
int AllocImageObj(image *f, int Width, int Height, int NumChannels)
{
    ...
    if(!(f->Data = (num *)malloc(sizeof(num)*Width*Height*NumChannels)))
    {
        *f = NullImage;
        return 0;
    }    
...
int ReadMatrixFromTextFile(image *f, const char *FileName)
{
    ...
    num *Dest = NULL;
    double Value;
    int c, Line = 1, Col = 0, NumRows = 0, NumCols = 0, DestNumEl = 0;
    int DestCapacity = 64;   /* Initial capacity */
        ...
        if(c == EOF || c == '\n' || c == '\r')
        {
            if(Col) /* End of a non-empty line */
            {
                ...
                else if(NumCols != Col)
                {
                    ...
                    fprintf(stderr, "Rows must have a consistent number of elements.\n");
                    goto Catch;
                }
