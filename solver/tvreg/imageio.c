...
static int ReadBmp(uint32_t **Image, int *Width, int *Height, FILE *File)
{
    ...
    *Width = *Height = 0;
    ...
    if(InfoSize < 12)
    {
        printf("Invalid BMP info header.\n");
        goto Catch;
    }
    ...
    {
        printf("Image dimensions exceed MAX_IMAGE_SIZE.\n");
        goto Catch;
    }
...
/**
* @brief Write a BMP image as uncompressed RGB data
*
...
*       widely supported.  On the other hand, RGB 24-bit BMP is well 
*       supported.
*/
static int WriteBmp(const uint32_t *Image, int Width, int Height, FILE *File)
{
    ...
    putc(0x4D, File);
    WriteDWordLE(54 + ImageSize, File);	/* Filesize                  */
    WriteDWordLE(0, File);				/* Reserved fields           */
    WriteDWordLE(54, File);  			/* Image data offset         */
    ...
    WriteWordLE(24, File);				/* Bits per pixel            */
    ...
    WriteDWordLE(0, File);				/* Number of colors          */
...
/**
* @brief Write a PNG image as RGBA data
*
* @param Image pointer to RGBA image data
* @param Width, Height the image dimensions
* @param File stdio FILE pointer
...
static int WritePng(const uint32_t *Image, int Width, int Height, FILE *File)
{
    ...
    if((RowPointers = (png_bytep *)malloc(sizeof(png_bytep)*Height)) == 0)
    {
        printf("Memory allocation failed.\n");
        return 0;
    }
    ...
    /* Configure output for 8-bit RGBA color data */
    png_init_io(Png, File);
    ...
    for(Row = 0; Row < (unsigned)Height; Row++)
        RowPointers[Row] = (png_bytep)(Image + Width*Row);
    ...
    const char *FileName, unsigned Directory)
{
    ...
    *Width = *Height = 0;
    ...
    {
        printf("TIFFOpen failed to open file.\n");
        return 0;
    }
...
* @param 1 on successful identification, 0 on failure.
* 
    ...
    const char *FileName, unsigned Format)
{
    ...
    if(!strcmp(Type, "BMP"))
        Success = ReadBmp(&ImageU8, Width, Height, File);
    else if(!strcmp(Type, "JPEG"))
    {
#ifdef LIBJPEG_SUPPORT
        Success = ReadJpeg(&ImageU8, Width, Height, File);
#else
        fprintf(stderr, "File is a JPEG image.  Compile with LIBJPEG_SUPPORT to enable JPEG reading.\n");
#endif
    }
    else if(!strcmp(Type, "PNG"))
    {
#ifdef LIBPNG_SUPPORT
        Success = ReadPng(&ImageU8, Width, Height, File);
#else
        fprintf(stderr, "File is a PNG image.  Compile with LIBPNG_SUPPORT to enable PNG reading.\n");
#endif
    }
...
#ifdef LIBTIFF_SUPPORT
        fclose(File);
        Success = ReadTiff(&ImageU8, Width, Height, FileName, 0);
        File = NULL;
#else
        fprintf(stderr, "File is a TIFF image.  Compile with LIBTIFF_SUPPORT to enable TIFF reading.\n");
#endif
    }
    else
    {
        ...
        if(Type[0])
            ...
            fprintf(stderr, "File \"%s\" is an unrecognized format.\n", FileName);
        fprintf(stderr, "Sorry, only " READIMAGE_FORMATS_SUPPORTED " reading is supported.\n");
    }
    ...
    if(ImageU8 && Format)
    {
        if(!(Image = ConvertToFormat(ImageU8, *Width, *Height, Format)))
    ...
    const char *FileName, unsigned Format, int Quality)
{
        ...
        || StringEndsWith(FileName, ".jpeg"))
    {
        ...
        fprintf(stderr, "Compile with LIBJPEG_SUPPORT to enable JPEG writing.\n");
        return 0;
#endif
    }
    else if(StringEndsWith(FileName, ".png"))
    {
        ...
        fprintf(stderr, "Compile with LIBPNG_SUPPORT to enable PNG writing.\n");
        return 0;
#endif
    }
        ...
        || StringEndsWith(FileName, ".tiff"))
    {
        ...
        fprintf(stderr, "Compile with LIBTIFF_SUPPORT to enable TIFF writing.\n");
        return 0;
#endif
    }
    else 
    {
        ...
        fprintf(stderr, "Sorry, only " WRITEIMAGE_FORMATS_SUPPORTED " writing is supported.\n");
        return 0;
    }
