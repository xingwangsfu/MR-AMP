function [out1,out2,out3,out4,out5,out6,out7,out8,out9] = ...
    gampEst_ver4(scaEstIn, scaEstOut, A, opt,x0)
...
while ~stop
    ...
    if (compVal)
	    ...
	    if (it==1), phat = Axhat; end
            valOut = sum(sum(scaEstOut.logScale(Axhat,pvar,phat)));
        end
        val = valOut + valIn;
%fprintf(1,'it=%i, val=%g, valOut=%g, valIn=%g\n',it,val,valOut,valIn)
    end
...
%                 stop = true;% remove the stopping criterion by tolerance
            elseif customStop==1 
    ...
    if (it==1)
        ...
        if any(isnan(xhatDampOpt)), % if user didn't specify opt.xhatDampPrev0
    ...
    % Step in rvar
    rvar = 1./A.multSqTr(svar);   % rvar = 1./((A.^2)*svar)  --> expend MN computations
    if rvarStep
        if (it==1)
    ...
    % Input linear stage
    rhat = xhatDamp + rvar.*(A.multTr(shat)); % rhat = xhat + rvar.*(A'*shat) --> expend MN computations
