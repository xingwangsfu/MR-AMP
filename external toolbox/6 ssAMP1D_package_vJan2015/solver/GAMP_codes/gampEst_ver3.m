function [out1,out2,out3,out4,out5,out6,out7,out8,out9] = ...
    gampEst_ver3(scaEstIn, scaEstOut, A, opt,x0)
% gampEst:  G-AMP estimation algorithm
% % modified by Jaewook Kang (jwkkang@gist.ac.kr at 2014 June 12th) for MSE
% over iterations comparison (Fig.9)
...
MSE_iter=zeros(nit,1);
residual_iter=zeros(nit,1);
...
%     if mod(it,50)==0
%         figure(1)
%         plot(1:n-1,diff(xhatFinal));hold on
%     end
%------------------------------
    ...
    if (pass)
        ...
        if (it > 1) && ...
            ...
            stop = true;
        elseif (it > 1) && customStop && stopFcn(val, xhatFinal, ...
                xhatPrevFinal, Axhat)
            stop = true;    % Custom stopping criterion satisfied
        end
        
        % Continued output step
        phat = Axhat - (A2xvar/scaleFac).*shat; %uses A2xvar rather than pvar
        %  "(A2xvar/scaleFac).*shat" takes a role of onsager term .
        ...
        %-----------------------------------------------------------------
        residual_iter(it)=norm(shatNew(m-n+1:end),2)^2;
        
%             %---------------------------
%         if mod(it,100)==0
%            figure(1)
%            plot(1:length(shatNew ),shatNew );hold on
%         end
%     %--------------------------
    ...
    %----------------------------------------------------------------- 
    MSE_iter(it)=norm(xhat -x0)^2/norm(x0)^2;
...
if (opt.removeMean)
    xhatFinal = xhatFinal(1:n-2,:);
    xhatPrevFinal = xhatPrevFinal(1:n-2,:);
    xvarFinal = xvarFinal(1:n-2,:);
    rhatFinal = rhatFinal(1:n-2,:);
    rvarFinal = rvarFinal(1:n-2,:);
    shatFinal = shatFinal(1:m-2,:);
    svarFinal = svarFinal(1:m-2,:);
    zhatFinal = zhatFinal(1:m-2,:);
    zvarFinal = zvarFinal(1:m-2,:);
end
...
if (legacyOut)
    ...
    estFin.nit = it;
    estFin.mse_iter = MSE_iter;
    estFin.residual_iter = residual_iter;
